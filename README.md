...
|---|---|
| SageMaker ml.t3.medium (~40 hrs active use) | ~$1.86 |
| KMS customer managed key | $1.00 |
| S3 storage (~500 MB) | ~$0.01 |
| Bedrock Data Automation (per workshop run) | ~$0.05 |
| **Total (active use)** | **~$2.92/month** |
| **Total (left running 24/7)** | **~$35/month** |

> **Tip:** Stop the notebook instance when not in use — it's the dominant cost driver. The KMS key is the fixed cost floor at $1/month.
