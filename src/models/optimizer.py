...
class SequentialOptimizer(BaseModel):
    ...
    Supports both flat and nested blueprint structures.
    """
    config: BDAConfig
    schema: Schema
    ...
    path_mapping: Dict[str, str] = Field(default_factory=dict, description="Path mapping for nested schema reconstruction")
    is_nested_blueprint: bool = Field(default=False, description="Whether the blueprint contains nested structures")
    ...
    def from_config_file(cls, config_file: str, threshold: float = 0.8, use_doc: bool = False, 
        ...
        if is_nested_blueprint:
            logger.info("🔄 Detected nested blueprint structure - flattening for optimization")
            schema, path_mapping = original_schema.flatten_for_optimization()
            logger.info(f"✅ Flattened {len(path_mapping)} nested field paths")
        else:
            logger.info("📋 Detected flat blueprint structure - proceeding with standard optimization")
            schema = original_schema
            path_mapping = {}
        
        # Initialize strategy manager with flattened field names
        ...
        return cls(
            ...
            is_nested_blueprint=is_nested_blueprint
        )
    ...
    def update_schema_with_instructions(self, instructions: Dict[str, str]) -> str:
        ...
        For nested blueprints, saves both flattened (for BDA processing) and 
        unflattened (for final export) versions.
        
        Args:
            instructions: Instructions by field name
            
        Returns:
            str: Path to updated schema file (flattened version for BDA processing)
        """
        # Update flattened schema with new instructions
        ...
        if self.is_nested_blueprint and self.path_mapping:
            try:
                unflattened_schema = self.original_schema.unflatten_from_optimization(
                    self.schema, self.path_mapping
                )
                nested_output_path = f"{run_dir}/schema_{self.iteration}_nested.json"
                unflattened_schema.to_file(nested_output_path)
                logger.info(f"✅ Nested schema saved to {nested_output_path}")
            except Exception as e:
                logger.warning(f"⚠️ Could not save nested schema version: {str(e)}")
        
        logger.info(f"✅ Schema updated and saved to {flattened_output_path}")
    ...
    def run(self, max_iterations: int = None) -> str:
        ...
        # Save final schema - reconstruct nested structure if needed
        final_schema_path = f"{schema_run_dir}/schema_final.json"
        
        if self.is_nested_blueprint and self.path_mapping:
            # Reconstruct the nested structure for the final schema
            try:
                final_nested_schema = self.original_schema.unflatten_from_optimization(
                    self.schema, self.path_mapping
                )
                final_nested_schema.to_file(final_schema_path)
                logger.info(f"✅ Final nested schema saved to {final_schema_path}")
                
                # Also save the flattened version for reference
                flattened_final_path = f"{schema_run_dir}/schema_final_flattened.json"
                self.schema.to_file(flattened_final_path)
                logger.info(f"✅ Final flattened schema saved to {flattened_final_path}")
            except Exception as e:
                logger.warning(f"⚠️ Could not reconstruct nested final schema: {str(e)}")
                logger.info("📋 Saving flattened schema as final version")
                self.schema.to_file(final_schema_path)
        else:
            # For flat blueprints, save as usual
            self.schema.to_file(final_schema_path)
        ...
        if all_fields_meet_threshold:
            ...
            # Use the appropriate schema format for BDA - nested if originally nested, flat otherwise
            schema_path_for_bda = final_schema_path
            if self.is_nested_blueprint:
                logger.info("📋 Using reconstructed nested schema for BDA blueprint update")
            else:
                logger.info("📋 Using flat schema for BDA blueprint update")
            self.bda_client.update_customer_blueprint(schema_path_for_bda)
