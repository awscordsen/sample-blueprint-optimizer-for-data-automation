...
interface InputDocumentFieldProps {
  value: string;
  onChange: (value: string) => void;
  onS3UriChange?: (s3Uri: string) => void;
  errorText?: string;
  invalid?: boolean;
  onErrorClear?: () => void;
}
...
const InputDocumentField: React.FC<InputDocumentFieldProps> = ({
  value,
  onChange,
  onS3UriChange,
  errorText,
  ...
  // Load S3 buckets when modal opens
  useEffect(() => {
    if (showUploadModal && buckets.length === 0) {
      loadS3Buckets();
    }
  }, [showUploadModal]);
  ...
  useEffect(() => {
    if (selectedBucket && showUploadModal) {
      validateBucketAccess();
    }
  }, [selectedBucket, showUploadModal]);
  ...
  const validateBucketAccess = async () => {
    ...
    setBucketValidation({ 
      status: 'validating', 
      message: 'Validating bucket access...', 
      hasReadAccess: false, 
      hasWriteAccess: false 
    });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        // Update the input field with the S3 URI
        onChange(data.s3_uri);
        
        // Notify parent component about the S3 URI change for auto-populating output location
        if (onS3UriChange) {
          onS3UriChange(data.s3_uri);
        }
  ...
  return (
    <>
      <FormField 
        ...
        constraintText="Required - Path or S3 URI to the input document"
        errorText={errorText}
      >
        <SpaceBetween direction="horizontal" size="xs">
          <Input
            ...
            placeholder="Path or S3 URI to the input document"
            invalid={invalid}
          />
          <Button
            variant="normal"
